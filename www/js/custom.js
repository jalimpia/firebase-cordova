...
function writeUserData(fname, lname, username, password, uploadfile) {
	...
	ref.once('value').then(function(snapshot) {
		...
		if(exist==false){
			...
			$("#result").empty();
			LoadData();
			$('#RegisterForm').find("input[type=text],input[type=password], textarea").val("");//Code for clearing form textboxes
		}else{
...
function Login(username,password){	
	...
	ref.once('value').then(function(snapshot) {
		...
		if(exist==true){
			//location.href="https://jalimpia-cs.firebaseapp.com/";
			
			sessionStorage.setItem("sessionId", SHA256(username+password));
